...
export default {
  ...
  data() {
    ...
    let formIsActive = this.admin ? true : this.questionnaire.active;

    let showForm = this.admin ? true : formIsActive;
    let showFormMessage = false;

    this.questionnaire.schema.forEach((schema) => {
      if (
      ...
      <div>
        <div class="flex flex-col lg:flex-row">
          ...
          <div
            ...
            class="lg:order-2 w-full sticky top-0 lg:relative lg:w-1/2 bg-white min-h-[55rem] max-h-[60rem] pt-[50px] pr-[0px] pb-[0px] pl-[50px] lg:h-[30rem] lg:p-[10px]"
          >
            <img
          ...
          <!-- Form Section -->
          <div class="bg-white lg:order-1 rounded-sm p-6 w-full lg:w-1/2 z-10">
            <div class="mb-7">
              <p class="text-2xl font-bold capitalize">{{ questionnaire.title }}</p>
              <p v-if="questionnaire.description" class="text-sm text-gray-600">
            ...
            <div id="form-container">
              <form @submit.prevent="submit" class="mt-4">
                <div class="space-y-6">
                  <div
                    v-for="(section, index) in questionnaire.schema"
                    :key="index"
                    class="border p-6 overflow-hidden shadow-sm sm:rounded-lg"
                  >
                    <div>
                      <p class="text-gray-800 text-xl font-bold capitalize">{{ section.title }}</p>
  ...
  <div v-if="showFormMessage && formIsActive">
    <div class="p-6">
      <p class="text-2xl font-bold capitalize text-green-500">
        {{ questionnaire.message_after_registration }}
      </p>
    </div>
    <iframe
      style="display: block; height: 98vh; width: 100vw"
      :src="admissionInstructionUrl"
      >Your browser isn't compatible</iframe
    >
  </div>
  ...
  <div class="p-6" v-if="!formIsActive">
    <div>
      <p class="text-2xl font-bold capitalize">
        {{ questionnaire.message_when_inactive }}
      </p>
    </div>
  </div>
</template>
