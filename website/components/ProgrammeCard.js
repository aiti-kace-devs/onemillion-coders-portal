...
import { FiClock, FiArrowRight, FiCheckCircle, FiX, FiLoader, FiGlobe, FiInfo, FiMonitor, FiMapPin, FiCalendar, FiSun, FiArrowLeft, FiChevronRight, FiAlertCircle, FiTarget, FiStar } from "react-icons/fi";
...
const ProgrammeCard = ({ programme, userId, centreId, token }) => {
  ...
  const modeStyles = {
    "Hybrid": { color: "bg-blue-50 text-blue-700 border-blue-100", icon: FiGlobe },
    "In Person": { color: "bg-red-50 text-red-700 border-red-100", icon: FiMapPin },
    "Online": { color: "bg-purple-50 text-purple-700 border-purple-100", icon: FiMonitor },
  };

  const currentMode = modeStyles[programme.mode_of_delivery] || { color: "bg-gray-50 text-gray-700 border-gray-100", icon: FiGlobe };
  ...
  const getLevelBars = (level) => {
    ...
    const fillColors = { beginner: '#15803D', intermediate: '#A16207', advanced: '#1D4ED8' };
    const normalizedLevel = level?.trim().toLowerCase();
    ...
    return (
      <svg viewBox="0 0 100 80" className="w-4 h-4" xmlns="http://www.w3.org/2000/svg">
        {allBars.map((bar, index) => (
          <rect key={index} x={bar.x} y={bar.y} width="22" height={bar.height} rx="3" ry="3" fill={index < filledCount ? filledColor : '#D1D5DB'} />
        ))}
      </svg>
    );
  };
        ...
        {steps.map((label, i) => {
          ...
          return (
            <React.Fragment key={label}>
              <div className={`flex items-center gap-1 ${isDone ? "text-green-500" : isCurrent ? "text-yellow-600" : "text-gray-300"}`}>
                <div className={`w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center ${isDone ? "bg-green-500 text-white" : isCurrent ? "bg-yellow-400 text-gray-900" : "bg-gray-100 text-gray-400"}`}>
    ...
    <div className="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-200 cursor-pointer group">
      ...
      <div className="relative w-full h-48 bg-gray-100">
        {programme.image && !imageError ? (
          <Image src={programme.image} alt={programme.title} fill className="object-cover group-hover:scale-105 transition-transform duration-200" onError={() => setImageError(true)} />
        ) : (
          <div className="absolute inset-0 bg-gradient-to-br from-gray-50 to-gray-100 flex items-center justify-center">
            <Image src="/images/one-million-coders-logo.png" alt="One Million Coders" width={120} height={40} className="opacity-15" />
          </div>
        )}
        <div className="absolute top-4 left-4">
          <span
            onClick={(e) => { e.stopPropagation(); if (window.location.pathname.startsWith('/programmes')) { router.push(`/programmes/category/${programme.category?.id}`); } }}
      ...
      <div className="p-6">
        ...
        <p className="text-gray-600 text-sm mb-4 line-clamp-2">{programme.job_responsible}</p>
            ...
            >
              ...
              {enrollSuccess && (
                <div className="text-center">
                  ...
                  <p className="text-gray-500 text-sm sm:text-base mb-2">
                    Successfully enrolled in <span className="font-semibold text-gray-700">{enrolledCourseName}</span>.
                  </p>
                  {selectedBatch && selectedSession && (
                    <p className="text-gray-400 text-xs sm:text-sm mb-6">{selectedBatch.name} · {selectedSession.label} ({selectedSession.time})</p>
                  )}
              ...
              {!enrollSuccess && waitlistJoined && (
                <div className="text-center">
                  ...
                  <h2 className="text-lg sm:text-2xl font-bold text-gray-900 mb-2">You&apos;re on the waitlist!</h2>
                  <p className="text-gray-500 text-sm sm:text-base mb-6">We&apos;ll notify you when a slot opens for <span className="font-semibold text-gray-700">{enrolledCourseName}</span>.</p>
                  <button onClick={closeEnrollmentModal} className="px-6 py-3 bg-yellow-400 hover:bg-yellow-500 text-gray-900 font-semibold text-sm rounded-xl transition-colors">Close</button>
                </div>
              )}
              ...
              {!enrollSuccess && !waitlistJoined && enrollmentStep === "batch" && (
                <>
                  ...
                  <p className="text-gray-400 text-[11px] sm:text-sm mb-4 sm:mb-5 line-clamp-1">{enrolledCourseName}</p>
              ...
              {!enrollSuccess && !waitlistJoined && enrollmentStep === "session" && (
                <>
                  ...
                  <button onClick={() => setEnrollmentStep("batch")} className="inline-flex items-center gap-1 text-[11px] text-gray-400 hover:text-gray-600 transition-colors mb-3"><FiArrowLeft className="w-3 h-3" /> Back</button>
                    ...
                    {MOCK_SESSIONS.map((session) => {
                      ...
                      return (
                        <button key={session.id} onClick={() => handleSessionSelect(session)}
                          ...
                          <div className="flex items-center gap-3">
                            <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${isSelected ? "bg-yellow-400" : "bg-gradient-to-br from-yellow-50 to-orange-50 group-hover:from-yellow-100 group-hover:to-orange-100"}`}>
              ...
              {!enrollSuccess && !waitlistJoined && enrollmentStep === "confirm" && (
                <>
                  ...
                  <button onClick={() => setEnrollmentStep("session")} className="inline-flex items-center gap-1 text-[11px] text-gray-400 hover:text-gray-600 transition-colors mb-3"><FiArrowLeft className="w-3 h-3" /> Back</button>
                  ...
                  <div className="mb-5 rounded-xl bg-gradient-to-br from-gray-50 to-gray-100/50 border border-gray-200 overflow-hidden">
                    ...
                    <div className="px-3 sm:px-4 py-2 space-y-1.5">
                      <div className="flex items-center justify-between text-[11px] sm:text-xs"><span className="text-gray-400">Batch</span><span className="font-medium text-gray-700">{selectedBatch?.name}</span></div>
                      <div className="flex items-center justify-between text-[11px] sm:text-xs"><span className="text-gray-400">Session</span><span className="font-medium text-gray-700">{selectedSession?.label} · {selectedSession?.time}</span></div>
                    </div>
                  </div>
                  {enrollError && <div className="mb-3 p-3 bg-red-50 border border-red-200 rounded-xl"><p className="text-red-700 text-sm">{enrollError}</p></div>}
                  <div className="flex items-center gap-2.5">
                    <button onClick={closeEnrollmentModal} className="flex-1 py-3 bg-gray-50 hover:bg-gray-100 text-gray-500 font-medium text-sm rounded-xl transition-colors">Cancel</button>
                    <button onClick={handleEnrollSubmit} disabled={enrollSubmitting}
              ...
              {!enrollSuccess && !waitlistJoined && enrollmentStep === "courseFull" && (
                <>
                  ...
                  <div className="mb-5">
                    ...
                    <div className="space-y-2">
                      {MOCK_ALT_CENTRES.map((alt) => (
                        <button key={alt.id} onClick={() => { setEnrollingCentreId(alt.id); setEnrollingCentreTitle(alt.name); setSelectedBatch(null); setSelectedSession(null); setEnrollmentStep("batch"); }}
                  ...
                  <div className="mb-5">
                    <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Other courses at {centreTitle}</h3>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {MOCK_ALT_COURSES.map((alt) => (
                        <button key={alt.id} onClick={() => { setEnrolledCourseName(alt.title); setSelectedBatch(null); setSelectedSession(null); setEnrollmentStep("batch"); }}
              ...
              {!enrollSuccess && !waitlistJoined && enrollmentStep === "batchFull" && (
                <>
                  ...
                  <button onClick={() => { setEnrollmentStep("batch"); setSelectedBatch(null); setSelectedSession(null); }}
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-yellow-50 to-orange-50 flex items-center justify-center flex-shrink-0 group-hover:from-yellow-100 group-hover:to-orange-100 transition-colors"><FiCalendar className="w-4 h-4 text-yellow-600" /></div>
                      <div className="flex-1 text-left"><div className="text-sm font-semibold text-gray-900 group-hover:text-yellow-700 transition-colors">Pick a different batch</div><div className="text-xs text-gray-400">Other batches may still have slots</div></div>
                    ...
                    {MOCK_ALT_CENTRES.map((alt) => (
                      <button key={alt.id} onClick={() => { setEnrollingCentreId(alt.id); setEnrollingCentreTitle(alt.name); setSelectedBatch(null); setSelectedSession(null); setEnrollmentStep("batch"); }}
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1 min-w-0"><div className="text-sm font-medium text-gray-900 group-hover:text-yellow-700 transition-colors truncate">{alt.name}</div></div>
