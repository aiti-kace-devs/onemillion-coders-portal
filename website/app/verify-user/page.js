...
function VerifyUserContent() {
  ...
  useEffect(() => {
    if (!token) {
      setAuthError("No authentication token provided. Please access this page from the student portal.");
    }
  }, [token]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setSubmitError(response.message || "Verification submission failed. Please try again.");
      }
    } catch (error) {
