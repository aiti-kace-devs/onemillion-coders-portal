...
export default {
  ...
  mounted() {
    ...
    $(document).on("click", ".edit", (evt) => {
      ...
      router.get(route("admin.questionnaire.edit", data));
    });

    $(document).on("click", ".preview", (evt) => {
      const data = $(evt.currentTarget).data("id");

      router.get(route("admin.questionnaire.preview", data));
    });

    $(document).on("click", ".responses", (evt) => {
      const data = $(evt.currentTarget).data("id");

      router.get(route("admin.questionnaire.show", data));
    });
  ...
  methods: {
    fetch() {
      $("#data_table").DataTable({
        ...
        serverSide: true,
        ajax: {
          url: route("admin.questionnaire.fetch"),
        },
        columns: [
    ...
    destroy() {
      axios
        .post(route("admin.questionnaire.destroy", { form: this.selectedRow }))
        .then((response) => [
...
<template>
  ...
  <AuthenticatedLayout>
    ...
    <template #header>
      <div class="flex items-center">Questionnaire</div>
    </template>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
          <div class="p-6">
            <div class="flex justify-end">
              <LinkButton :href="route('admin.questionnaire.create')">
