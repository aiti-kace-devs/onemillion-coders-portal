...
const fetchedQuestions = ref([]); // Reactive ref for fetched questions from AJAX
const showUnansweredHighlight = ref(false); // Controls whether unanswered questions are highlighted

// Form will be initialized in startTest after fetching questions
let form = useForm({
        ...
        .then((response) => {
            if (response.data.status) {
                ...
                if (Array.isArray(questionsData)) {
                    // Store fetched questions in reactive ref
                    fetchedQuestions.value = questionsData;

                    // Build form data object with all answer fields
                    const formData = {
                        exam_id: props.exam.id,
                        index: questionsData.length,
                    };

                    // Add question IDs and answer fields
                    questionsData.forEach((q, idx) => {
                        formData[`question${idx + 1}`] = q.id;
                        formData[`ans${idx + 1}`] = null;
                    });

                    // Reinitialize the form with all fields
...
function confirmSubmit() {
    const questionCount = fetchedQuestions.value.length;
    const unansweredQuestions = [];
    const answeredQuestions = [];

    Object.entries(form.data()).forEach(([key, value]) => {
        if (key.startsWith("ans")) {
            const unansweredNumber = key.replace("ans", "");

            if (value !== null && value !== undefined && value !== "") {
                answeredQuestions.push(key);
            } else {
                unansweredQuestions.push(`Q${unansweredNumber}`);
            }
        }
    });

    // After first submit attempt, start highlighting unanswered questions
    showUnansweredHighlight.value = true;

    const remainder = questionCount - answeredQuestions.length;

    modalType.value = "submission";
    modalMessage.value = `Are you sure you want to submit this test? This cannot be undone. ${remainder !== 0
        ? `${remainder} question${remainder !== 0 ? "s" : ""} left to answer: ${unansweredQuestions.join(", ")}`
        : ""
        }`;
    ...
    <div id="exam-content" class="bg-gray-50 py-6 px-2 sm:px-6 lg:px-8">
        <div v-if="!modalShow || modalType !== 'instructions'" class="max-w-4xl mx-auto">
            ...
            <div
                ...
                <!-- Timer display -->
                <div class="order-1 flex items-center text-center sm:mb-0 space-x-1"
                ...
                <div class="w-full md:w-auto text-center order-3 md:order-2">
                    <PrimaryButton type="button" @click="confirmSubmit" :disabled="form.processing">SUBMIT TEST
                    </PrimaryButton>
                </div>

                <!-- Status indicator -->
                <div class="order-2 md:order-3 flex items-center text-center sm:text-right space-x-2">
                    <div>
            ...
            <div>
                <form id="examination_form"
                    ...
                    <div v-for="(q, idx) in fetchedQuestions" :key="q.id" :class="[
                        'rounded-lg p-4 mb-4 bg-white border-2',
                        showUnansweredHighlight && !form[`ans${idx + 1}`]
                    ...
                    <div class="text-center">
                        <PrimaryButton type="button" @click="confirmSubmit">SUBMIT TEST</PrimaryButton>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="p-6">
            ...
            <template v-else-if="modalType === 'warning'">
                ...
                <div class="mt-6 flex justify-center">
                    <PrimaryButton v-if="warn < 3" @click="handleModalContinue">Continue</PrimaryButton>
            ...
            <template v-else-if="modalType === 'submission'">
                ...
                <p class="mt-1 text-sm text-center text-gray-600" v-html="modalMessage"></p>
                <div class="mt-6 flex justify-center">
                    <SecondaryButton @click="() => { modalShow = false; highlightedQuestions = new Set(); }"
                        :disabled="form.processing">Cancel</SecondaryButton>
