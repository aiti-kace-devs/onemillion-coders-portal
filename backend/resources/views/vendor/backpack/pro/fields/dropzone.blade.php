@php
    ...
    $defaultConfig = [
        'url' => $field['configuration']['url'] ??  url($crud->route.'/ajax-upload'),
    ...
    $field['configuration']['deleteUrl'] = $field['configuration']['deleteUrl'] ??  url($crud->route.'/ajax-upload/delete');
    ...
    $temporaryDisk = CRUD::get('ajax-upload.temporary_disk');
    $temporaryDirectory = CRUD::get('ajax-upload.temporary_folder');
    ...
    if (is_array($field['value'])) {
        ...
        foreach ($field['value'] as $key => $path) {
            $isTemporaryFile = strpos($path, $temporaryDirectory) !== false;
            $disk = $isTemporaryFile ? $temporaryDisk : $field['disk'];
            try {
                $serverFiles[] = [
                    'name' => $isTemporaryFile ? $path : basename($path),
...
@include('crud::fields.inc.wrapper_start')
    ...
    <div
        ...
        data-form-operation="{{ $crud->get('ajax-upload.formOperation') }}"
        data-temp-upload-folder-name="{{ $temporaryDirectory }}"
        data-is-dropzone-active="{{ var_export(($disabled || $readonly) ? false : true) }}"
        >
        <div class="dz-message">
            <button class="dz-button" type="button">{!! trans('backpack/pro::uploaders.click_or_drop_files') !!}</button>
        </div>
    </div>
...
{{-- If a field type is shown multiple times on a form, the CSS and JS will only be loaded once --}}
    ...
    @push('crud_fields_styles')
        ...
        <link href="https://cdn.jsdelivr.net/npm/dropzone@6.0.0-beta.2/dist/dropzone.css" rel="stylesheet" crossorigin="anonymous">
    ...
    @push('crud_fields_scripts')
        {{-- include dropzone js --}}
        <script src="https://cdn.jsdelivr.net/npm/dropzone@6.0.0-beta.2/dist/dropzone-min.js" crossorigin="anonymous"></script>
        ...
        <script>
            function bpFieldInitDropzoneElement(element) {
                ...
                $dropzoneConfig.hiddenInputContainer = dropzoneHiddenContainer;
                ...
                $dropzoneConfig.removedfile = function (file, xhr) {
                    ...
                    if (!filePath.path || filePath.path.includes(tempUploadFolderName)) {
                        $.ajax({
                            url: $dropzoneConfig.deleteUrl,
                            type: 'POST',
                            data: `operation=${formOperation}&fieldName=${$dropzoneConfig.paramName}&file=${filePath}`,
                            ...
                            error: function (data) {
                                ...
                                let errorContainer = document.createElement("div");
                                errorContainer.classList.add('invalid-feedback', 'd-block');
                                errorContainer.innerHTML = data.responseJSON.message;
                                dz.parentNode.appendChild(errorContainer);
                            }
                        });
                    } else {
