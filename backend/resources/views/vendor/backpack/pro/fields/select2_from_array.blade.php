...
@php
    ...
    $field['multiple'] = $field['multiple'] ?? $field['allows_multiple'] ?? false;
    ...
    $field['tagging'] = $field['tagging'] ?? false;

    // Add any values from $field['value'] that don't exist in $field['options'] as options
    $options = $field['options'] ?? [];

    if ($field['sortable']) {
        $optionsFromValue = [];
        foreach (\Illuminate\Support\Arr::wrap($field['value']) as $key) {
            if (!is_array($key) && $key !== '') {
                $optionsFromValue[$key] = $options[$key] ?? $key;
            }
        }
        $options = $optionsFromValue + $options;
    } else {
        foreach (\Illuminate\Support\Arr::wrap($field['value']) as $key) {
            if (!isset($options[$key]) && !is_array($key) && $key !== '') {
                $options[$key] = $key;
            }
        }
    }
...
@include('crud::fields.inc.wrapper_start')
    ...
    <select
        ...
        data-tagging="{{var_export($field['tagging'])}}"
...
<script src="https://cdn.jsdelivr.net/npm/jquery-ui@1.13.2/dist/jquery-ui.min.js" crossorigin="anonymous"></script>
@if (app()->getLocale() !== 'en')
    ...
    function bpFieldInitSelect2FromArrayElement(element) {
        ...
        if (!element.hasClass("select2-hidden-accessible"))
        {
            ...
            let select2Options = {
                ...
                tags: element.data('tagging') == true,
            };

            let select2 = element.select2(select2Options);
