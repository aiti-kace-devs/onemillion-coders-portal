@if ($crud->hasAccess('destroy', $entry) && ($entry->trashed() || (!$entry->trashed() && $crud->getOperationSetting('canDestroyNonTrashedItems'))))
	<a href="javascript:void(0)" onclick="deleteEntryPermanently(this)" bp-button="destroy" data-route="{{ url($crud->route.'/'.$entry->getKey().'/destroy') }}" data-table-id="{{ isset($crudTableId) ? $crudTableId : 'crudTable' }}" class="btn btn-sm btn-link" data-button-type="destroy"><i class="la la-fire-alt"></i> <span>{{ trans('backpack/pro::trash.destroy') }}</span></a>
@endif
