@if ($crud->hasAccess('bulkDelete') && $crud->get('list.bulkActions'))
	<a href="javascript:void(0)" onclick="bulkDeleteEntries(this)" bp-button="bulkDelete" data-table-id="{{ isset($crudTableId) ? $crudTableId : 'crudTable' }}" class="btn btn-sm btn-secondary bulk-button"><i class="la la-trash"></i> <span>{{ trans('backpack::crud.delete') }}</span></a>
@endif
...
<script>
	if (typeof bulkDeleteEntries != 'function') {
	  function bulkDeleteEntries(button) {
	      ...
	      var message = ("{!! trans('backpack::crud.bulk_delete_are_you_sure') !!}").replace(":number", checkedItems.length);
	      var button = $(this);
					...
					$.ajax({
						...
						success: function(result) {
							if (Array.isArray(result)) {
							  ...
							  new Noty({
							    type: "success",
							    text: "<strong>{!! trans('backpack::crud.bulk_delete_sucess_title') !!}</strong><br>"+checkedItems.length+"{!! trans('backpack::crud.bulk_delete_sucess_message') !!}"
							  }).show();
							} else {
