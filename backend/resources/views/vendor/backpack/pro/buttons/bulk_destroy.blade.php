@if ($crud->hasAccess('bulkDestroy') && $crud->get('list.bulkActions'))
	<a href="javascript:void(0)" onclick="bulkDestroyEntries(this)" bp-button="bulkDestroy" data-table-id="{{ isset($crudTableId) ? $crudTableId : 'crudTable' }}" class="btn btn-sm btn-secondary bulk-button trash-button bulk-destroy-button"><i class="la la-fire-alt"></i> <span>{{ trans('backpack/pro::trash.destroy') }}</span></a>
@endif
...
<script>
	if (typeof bulkDestroyEntries != 'function') {
	  function bulkDestroyEntries(button) {
	      ...
	      var message = ("{!! trans('backpack/pro::trash.bulk_destroy_confirm') !!}").replace(":number", checkedItems.length);
	      var button = $(this);
					...
					$.ajax({
						...
						success: function(result) {
							if (Array.isArray(result)) {
							  ...
							  new Noty({
							    type: "success",
							    text: "<strong>{!! trans('backpack/pro::trash.bulk_destroy_success_title') !!}</strong><br>"+checkedItems.length+"{!! trans('backpack/pro::trash.bulk_destroy_success_message') !!}"
							  }).show();
							} else {
