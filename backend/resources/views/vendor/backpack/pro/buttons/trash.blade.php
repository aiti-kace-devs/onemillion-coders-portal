@if ($crud->hasAccess('trash', $entry) && $entry->trashed() === false)
	<a href="javascript:void(0)" onclick="trashEntry(this)" bp-button="trash" data-route="{{ url($crud->route.'/'.$entry->getKey().'/trash') }}" data-table-id="{{ isset($crudTableId) ? $crudTableId : 'crudTable' }}" class="btn btn-sm btn-link" data-button-type="trash"><i class="la la-trash"></i> <span>{{ trans('backpack/pro::trash.trash') }}</span></a>
@endif
