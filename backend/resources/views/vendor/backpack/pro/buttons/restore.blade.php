@if ($crud->hasAccess('restore', $entry) && $entry->trashed())
	<a href="javascript:void(0)" onclick="restoreEntry(this)" bp-button="restore" data-route="{{ url($crud->route.'/'.$entry->getKey().'/restore') }}" data-table-id="{{ isset($crudTableId) ? $crudTableId : 'crudTable' }}" class="btn btn-sm btn-link" data-button-type="restore"><i class="la la-recycle"></i> <span>{{ trans('backpack/pro::trash.restore') }}</span></a>
@endif
