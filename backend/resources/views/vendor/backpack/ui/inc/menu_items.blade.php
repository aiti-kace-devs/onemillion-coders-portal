...
@if (auth()->user()->can('student.read.all') || auth()->user()->can('student-verification.read.all'))
    <x-backpack::menu-dropdown title="Student Management" icon="la la-users">
        @can('student.read.all')
            <x-backpack::menu-dropdown-item title="Students" icon="la la-user-graduate" :link="backpack_url('manage-student')" />
            <x-backpack::menu-dropdown-item title="User Accounts" icon="la la-user-alt" :link="backpack_url('user')" />
        @endcan
