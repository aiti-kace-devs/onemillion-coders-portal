...
@else
    <span class="crud_bulk_actions_checkbox">
        <input type="checkbox" class="crud_bulk_actions_line_checkbox form-check-input"
    ...
    @bassetBlock('backpack/crud/operations/list/bulk-actions-checkbox.js')
        <script>
                            ...
                            if (checked) {
                                ...
                                if (tableConfig.lastCheckedItem && e.shiftKey) {
                                    ...
                                    let first = document.querySelector(
                                        `#${tableId} input.crud_bulk_actions_line_checkbox[data-primary-key-value="${tableConfig.lastCheckedItem}"]`
                                        ).closest('tr');
                                    let last = document.querySelector(
                                        `#${tableId} input.crud_bulk_actions_line_checkbox[data-primary-key-value="${primaryKeyValue}"]`
                                        ).closest('tr');
                                    let firstIndex = getNodeindex(first);
                                    let lastIndex = getNodeindex(last);

                                    while (first !== last) {
                                        first = firstIndex < lastIndex ? first.nextElementSibling : first
                                            .previousElementSibling;
                                        first.querySelector(
                                                'input.crud_bulk_actions_line_checkbox:not(:checked)')
                                            ?.click();
                                    }
                                }
                ...
                window.crud.markCheckboxAsCheckedIfPreviouslySelected = function(tableId = 'crudTable') {
                    ...
                    document
                        ...
                        .forEach(function(elem) {
                            let checked = checkedItems.length && checkedItems.indexOf(elem.dataset.primaryKeyValue) > -
                            1;
            ...
            window.crud.addBulkActionMainCheckboxesFunctionality = function(tableId = 'crudTable') {
                ...
                let mainCheckboxes = Array.from(document.querySelectorAll(
                    `#${tableId} input.crud_bulk_actions_general_checkbox`));

                mainCheckboxes.forEach(checkbox => {
                    const getRowCheckboxes = () => Array.from(document.querySelectorAll(
                        `#${tableId} input.crud_bulk_actions_line_checkbox`));
                    ...
                    const updateMainCheckboxState = () => {
                        ...
                        checkbox.checked = rowCheckboxes.length > 0 &&
                            document.querySelectorAll(
                                `#${tableId} input.crud_bulk_actions_line_checkbox:not(:checked)`).length === 0;
                    };
