...
if (isset($field['parentFieldName'])) {
    if (!empty(old())) {
        $field['value'] =
            Arr::get(old(), square_brackets_to_dots($field['name'])) ??
    ...
    <div class="existing-file">
        @if (isset($field['disk']))
            @if (isset($field['temporary']))
                <a target="_blank"
                    href="{{ asset(\Storage::disk($field['disk'])->temporaryUrl(Arr::get($field, 'prefix', '') . $field['value'], Carbon\Carbon::now()->addMinutes($field['expiration']))) }}">
                @else
                    <a target="_blank"
...
<div class="backstrap-file {{ isset($field['value']) && $field['value'] != null ? 'd-none' : '' }}">
    <input type="file" name="{{ $field['name'] }}" data-filename="{{ $field['value'] ?? '' }}"
