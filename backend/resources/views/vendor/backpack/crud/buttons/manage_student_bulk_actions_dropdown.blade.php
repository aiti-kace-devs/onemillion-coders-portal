...
{{-- reuse the same Admit / Choose Session modals as on the preview page --}}
@include('vendor.backpack.crud.modals.manage_student_modals')
...
@push('after_scripts')
    ...
    @bassetBlock('custom/js/bulk_shortlist_action.js')
        <script>
            ...
            // Bulk Admit – confirm, then open Admit modal to pick course & session
            $('#bulkAdmitBtn').on('click', function(e) {
                e.preventDefault();
                const ids = getCheckedStudentIds();
                const applyToAll = shouldApplyToAll();
                ...
                Swal.fire({
                    ...
                    text: `You are about to admit ${totalCount} student(s). Continue?`,
                    icon: 'question',
                    ...
                    bulkAdmitIds = ids;
                    bulkAdmitApplyToAll = applyToAll;

                    // reset modal state
                    $('#admit_user_id').val('');
                    $('#admit_change').val('false');
                    $('#admitModalLabel').text('Admit Students');
                    $('#admit_form_submit').text('Admit');

                    // load courses (same endpoint as preview)
                    $.get("{{ url(config('backpack.base.route_prefix').'/course/ajax-list') }}", function(courses) {
                        const selectEl = $('#admit_course_id');
                        selectEl.empty().append('<option value=\"\">Select Course</option>');
                        courses.forEach(function(c) {
                            selectEl.append($('<option></option>').val(c.id).text(c.course_name));
                        });
                    });
                    $('#admit_session_id').empty()
                        .append('<option value=\"\">Select course first</option>')
                        .prop('disabled', true);
            ...
            // Bulk admit submit: reuse Admit modal submit button, but detect list page
            $(document).on('click', '#admit_form_submit', function(e) {
                // if we're not on the list page, let the preview-page JS handle it
                const onListPage = $('#crudTable').length > 0;
                ...
                if (!courseId) {
                    if (typeof Swal !== 'undefined') Swal.fire('Error', 'Please select a course.', 'error');
                ...
                if (bulkAdmitApplyToAll) {
                    data.select_all_in_query = 1;
                } else if (bulkAdmitIds.length > 0) {
                    data.user_ids = bulkAdmitIds;
                }
                    ...
                    success: function(resp) {
                        if (typeof Swal !== 'undefined') {
                            Swal.fire({
                                icon: 'success',
                                title: 'Success',
                                text: resp.message || 'Students admitted successfully!'
                            }).then(() => window.location.reload());
                        } else {
                            alert(resp.message || 'Students admitted successfully!');
                            window.location.reload();
                        }
                    },
                    error: function(xhr) {
                        let errorMsg = xhr.responseJSON?.message || 'Failed to admit students.';
                        if (typeof Swal !== 'undefined') {
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                ...
                const admitModal = bootstrap.Modal.getInstance(admitModalEl);
                if (admitModal) admitModal.hide();
            });
