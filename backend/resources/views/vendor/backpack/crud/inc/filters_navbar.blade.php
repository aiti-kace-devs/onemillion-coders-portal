<nav class="navbar navbar-expand-lg navbar-filters mb-0 py-0 shadow-none" componentId="{{ $componentId ?? '' }}"
    ...
    <button class="navbar-toggler ms-3" type="button" data-toggle="collapse" {{-- for Bootstrap v4 --}}
        ...
        data-bs-target="#bp-filters-navbar" {{-- for Bootstrap v5 --}} aria-controls="bp-filters-navbar"
        aria-expanded="false" aria-label="{{ trans('backpack::crud.toggle_filters') }}">
    ...
    <div class="collapse navbar-collapse" id="bp-filters-navbar">
        <ul class="nav navbar-nav">
            ...
            <li class="nav-item"><a href="#"
                    class="nav-link remove_filters_button {{ count(Request::input()) != 0 ? '' : 'invisible' }}"><i
                        class="la la-eraser"></i> {{ trans('backpack::crud.remove_filters') }}</a></li>
        </ul>
    </div>{{-- /.navbar-collapse --}}
</nav>
...
@push('after_scripts')
    ...
    <script>
        ...
        if (typeof updateDatatablesOnFilterChange !== 'function') {
            function updateDatatablesOnFilterChange(filterName, filterValue, update_url = false, debounce = 500, tableId =
                'crudTable') {
                ...
                if (update_url) {
                    ...
                    callFunctionOnce(function() {
                        refreshDatatablesOnFilterChange(new_url, tableId)
            ...
            filtersNavbar.forEach(function(navbar) {
                ...
                document.addEventListener('backpack:filter:changed', function(event) {
                    ...
                    if (anyActiveFilters === true) {
                        navbar.querySelector('.remove_filters_button').classList.remove(
                            'invisible');
                    } else {
