...
class DynamicFileManagerConfig implements ConfigRepository
{
    /**
     * LFM Route prefix
    ...
    {
        $decryptedOptions = MediaHelper::decryptDiskOptions();

        $allowedTypes = $decryptedOptions['allowed_types'] ?? [];
    ...
    public function getDiskList(): array
    {
        ...
        $selectedDisks = $decryptedOptions['disks'] ?? [];

        if (count($selectedDisks)) {
            $disks = array_merge($disks, $selectedDisks);
        } else {
            $disks = ['public'];
        }

        if (backpack_user()->hasRole('super-admin')) {
            $disks =  array_merge($disks, [config('filesystems.cloud_disk', 'gcs')]);
        }

        return count($disks) > 0 ? $disks : ['public'];
    }
}
